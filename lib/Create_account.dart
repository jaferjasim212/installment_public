...
void main() {
  runApp(const MaterialApp(
    ...
    home: EmailSignUpStep(),
  ));
}

class EmailSignUpStep extends StatefulWidget {
  const EmailSignUpStep({super.key});

  @override
  State<EmailSignUpStep> createState() => _EmailSignUpStepState();
}

class _EmailSignUpStepState extends State<EmailSignUpStep> {
...
class _MapAndImageSelectionScreenState
  ...
  Future<void> _submitUserData() async {
    ...
    try {
      // 1. إنشاء الحساب داخل Supabase Auth
      ...
      await OneSignal.login(user.id); // ← تسجيل الدخول وربط المستخدم

      // 2. تحويل صورة المستخدم إلى base64
      String? base64Image;
      ...
      // 4. إدراج بيانات المستخدم
      await supabase.from('users_full_profile').insert({
        'id': user.id,
        'email': widget.email,
      ...
      final paymentInsert = await supabase.from('subscription_payments').insert({
        'user_id': user.id,
        'subscription_type': 'monthly', // ← نوع افتراضي (حتى لو Free Trial)
        'amount_paid': 0,
        'payment_method': 'Free Trial',
        'status': 'successful',
        'paid_at': DateTime.now().toIso8601String(),
        'note': 'تم منح فترة تجريبية مجانية لمدة 15 يوم',
      }).select().single();

      final paymentId = paymentInsert['id'];
      ...
      print("❌ خطأ: $e");
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text("❌ حدث خطأ: هذا الحساب تم انشائه مسبقاً.."),
        backgroundColor: Colors.red,
      ));
    } finally {
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            ElevatedButton(
              onPressed: _saving ? null : _submitUserData,
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.teal.shade800,
