...
class TodayInstallmentsDialog extends StatefulWidget {
  const TodayInstallmentsDialog({super.key});

  @override
  State<TodayInstallmentsDialog> createState() => _TodayInstallmentsDialogState();
}

class _TodayInstallmentsDialogState extends State<TodayInstallmentsDialog>
  ...
  void initState() {
    ...
    _loadTodayInstallments();
  }


  Future<void> _loadTodayInstallments() async {
    ...
    final response = await Supabase.instance.client
      ...
      ''')
        ...
        .eq('due_date', today)  // مقارنة مباشرة مع التاريخ الحالي
        .order('due_date');
                              ...
                              SliverList(
                                delegate: SliverChildBuilderDelegate(
                                    ...
                                    final diff = dueDate != null
                                        ...
                                        : 0;
                                    final statusText = diff < 0 ? 'متأخر ${-diff} يوم' : 'مستحق اليوم';
            ...
            for (var installment in lateInstallments) {
              if (selectedIds.contains(installment['id'])) {
                ...
                if (response != null && response['customer_profile_id'] != null) {
                  ...
                  final message = '''
لديك قسط مستحق للتسديد اليوم عن المنتج "$itemType" 🧾
